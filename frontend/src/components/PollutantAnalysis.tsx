...
import { Activity, Wind, Factory, AlertCircle, Box, Cpu } from "lucide-react";
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-5 mb-10">
             ...
             <div>
                <h4 className="text-white text-xl font-black tracking-tight uppercase">Dominant Risk <span className="text-red-600">Driver</span></h4>
                <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.3em] mt-1.5 opacity-60">SHAP Decomposition Factor</p>
             </div>
          </div>
          
          <div className="flex items-end gap-4 mb-6">
             <span className="text-white text-7xl font-black tracking-tighter tabular-nums" style={{ color, textShadow: `0 0 40px ${color}44` }}>{pollutant}</span>
             <span className="text-slate-500 text-xs font-black uppercase pb-2 tracking-[0.4em] opacity-40">Load Unit</span>
          </div>

          <p className="text-slate-400 text-sm leading-relaxed font-bold max-w-sm opacity-80">
            Neural sensors indicate <span className="text-white">{pollutant}</span> as the high-impact coefficient dictating the {category.toLowerCase()} atmospheric state.
          </p>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-5 mb-10">
             ...
             <div>
                <h4 className="text-white text-xl font-black tracking-tight uppercase">Primary Source <span className="text-red-600">Factor</span></h4>
                <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.3em] mt-1.5 opacity-60">NMF Latent Feature Engine</p>
             </div>
          </div>
          ...
          <div className="mt-10 flex items-center gap-4">
             <div className="flex -space-x-2">
                {[...Array(3)].map((_, i) => (
                   <div key={i} className="w-8 h-8 rounded-full border-2 border-[#0A0A0A] bg-white/10 flex items-center justify-center overflow-hidden">
                      <Cpu className="w-4 h-4 text-slate-500" />
                   </div>
                ))}
             </div>
             <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">3 Neural Clusters Voting</span>
          </div>
        </div>
